...
- [ ] `npm run build` проходит без ошибок после всех изменений
- [ ] Папка `tech/` настроена как отдельный content plugin, URL `/tech/[id]`
- [ ] Папка `tasks/` настроена как отдельный content plugin, URL `/tasks/[id]`
- [ ] 3 демо-технологии (разные `tech_type`): standard, tool, methodology
- [ ] 2 демо-задачи (разные `difficulty`): Strong Junior, Middle
- [ ] Карта знаний `/map` отображает 3 типа узлов с разными формами/цветами
- [ ] Фильтры на карте работают (по типу, по tech_type, по difficulty)
- [ ] На странице статьи видны блоки "Используемые технологии" и "Практические задачи"
- [ ] На странице технологии видны блоки "Что нужно знать", "Где применяется", "Альтернативы", "Ссылка на оригинал"
- [ ] На странице задачи видны блоки "Необходимые знания", "Следующие задачи", "Типичные ошибки"
- [ ] Бейдж "🆕 New" отображается для технологий с `first_seen` ≤ 3 года от текущего
- [ ] Поиск находит технологии и задачи по ключевым словам
