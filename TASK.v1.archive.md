...
### 5.1. Созданы `paths/` с markdown-файлами треков: **ГОТОВО**
- `paths/junior-track.md` — 5 статей от простого к сложному
- `paths/api-track.md` — 3 статьи по API-интеграциям

### 5.2. Создана страница `src/pages/tracks.tsx` — список треков **ГОТОВО**
- Отображает все треки с названием, описанием, количеством статей
- Кнопка "Открыть трек" → детальный просмотр

### 5.3. Детальная страница трека через `?track=` (Docusaurus 3 не поддерживает file-based динамические маршруты) **ГОТОВО**
- Показывает нумерованный список статей
- Каждая статья — ссылка на `/docs/<id>`
- Ссылка "← Все треки" для возврата

**Коммит:** `feat: add learning tracks` — выполнен
