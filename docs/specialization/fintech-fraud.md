...
Транзакция
    ...
    ▼
┌─────────────────────┐
│     Rules Engine    │ ← статические правила (блоки, проверки)
└──────────┬──────────┘
           ▼
┌─────────────────────┐
│   ML Scoring Model  │ ← ML-модель: вероятность фрода (0..1)
└──────────┬──────────┘
           ▼
┌─────────────────────┐
│  Decision: Approve  │ ← Approve / Decline / Manual Review
│   / Decline / Hold  │
└──────────┬──────────┘
           ▼
   ┌─── Approve ───→ транзакция проходит
   │
   ├─── Decline ───→ блокировка, уведомление
   │
   └─── Hold ─────→ Case Management (ручной разбор)
```
