...
- Покупатель → «Просмотреть каталог»
- Покупатель → «Оформить заказ» `<<include>>` → «Оплатить заказ»
- Покупатель → «Отменить заказ»
- Менеджер → «Обновить каталог»
- Менеджер → «Обработать возврат»
- «Оформить заказ» `<<extend>>` ← «Применить промокод»
